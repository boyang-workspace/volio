...
function Toolbar() {
  ...
  return (
    <div className="flex items-center gap-3 px-6 py-3 border-b border-[#e5e5e5] shrink-0 min-h-[56px] bg-[#f5f5f7]/80 backdrop-blur-sm">
      <div className="flex-1 min-w-0">
        <input
          value={searchQuery}
          onChange={e => setSearchQuery(e.target.value)}
          placeholder={t('search')}
          className="h-9 w-full max-w-xs rounded-xl border border-[#d9d9de] bg-white px-3 text-sm text-[#1d1d1f] outline-none transition placeholder:text-[#a1a1a6] focus:border-[#007aff] focus:ring-3 focus:ring-[#007aff]/10"
        />
      </div>
      <div className="flex gap-0.5 shrink-0 rounded-xl overflow-hidden bg-[#e5e5e5]/50 p-0.5">
        {(['masonry', 'timeline']).map(v => (
          <button
            key={v}
            onClick={() => setCurrentView(v)}
            className={
              'h-7 px-3 text-[11px] font-medium rounded-[10px] border-0 transition-all duration-150 capitalize cursor-pointer ' +
              (currentView === v
